...
begin
    ...
    dut : entity tinymcu.tinymcu_periph_timer
        port map (clk_i => clk, rst_i => rst, irq_o => irq,
                   timer_req_i => req, timer_rsp_o => rsp);
    ...
    begin
        ...
        check("prescaler DIV4: 100 cycles -> 25 ticks", to_integer(unsigned(rd)) = 25);
        ...
        check("INT_STATUS clear before match", to_integer(unsigned(rd)) = 0);
        check("irq_o low before match", irq = '0');

        wait for CLK_PERIOD * 60;  -- 60 cycles @ DIV4 -> 15 ticks, well past compare=10

        bus_read(x"00000008", rd);
        check("INT_STATUS set after match", rd(0) = '1');
        check("irq_o high after match", irq = '1');

        -- Software clear.
        bus_write(x"00000008", x"00000000");        -- INT_STATUS = 0 (clear)
        bus_read(x"00000008", rd);
        check("INT_STATUS cleared by software write", to_integer(unsigned(rd)) = 0);
        check("irq_o low after clear", irq = '0');
        ...
        bus_read(x"00000008", rd);
        check("INT_STATUS stays clear (no spurious re-trigger)", to_integer(unsigned(rd)) = 0);
        ...
        bus_read(x"00000008", rd);
        check("INT_STATUS stays clear when INT_CONFIG disabled", to_integer(unsigned(rd)) = 0);
        check("irq_o stays low when INT_CONFIG disabled", irq = '0');
